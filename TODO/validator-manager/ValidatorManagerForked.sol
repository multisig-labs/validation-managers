...
abstract contract ValidatorManagerForked is Initializable, ContextUpgradeable, IValidatorManager {
  ...
  bytes32 public constant VALIDATOR_MANAGER_STORAGE_LOCATION = 0xe92546d698950ddd38910d2e15ed1d923cd0a7b3dde9e2a6a3f380565559cb00;
   ...
   */
  function _getValidatorManagerStorage() internal pure returns (ValidatorManagerStorage storage $) {
  ...
  function __ValidatorManager_init(ValidatorManagerSettings calldata settings) internal onlyInitializing {
    __Context_init();
    __ValidatorManager_init_unchained(settings);
  }

  // solhint-disable-next-line func-name-mixedcase
  function __ValidatorManager_init_unchained(ValidatorManagerSettings calldata settings) internal onlyInitializing {
    ValidatorManagerStorage storage $ = _getValidatorManagerStorage();
    $._l1ID = settings.l1ID;

    if (settings.maximumChurnPercentage > MAXIMUM_CHURN_PERCENTAGE_LIMIT || settings.maximumChurnPercentage == 0) {
  ...
  function initializeValidatorSet(ConversionData calldata conversionData, uint32 messageIndex) external {
    ...
    for (uint32 i; i < numInitialValidators; ++i) {
      ...
      emit InitialValidatorCreated(validationID, initialValidator.nodeID, initialValidator.weight);
    }
    ...
    bytes32 conversionID = ValidatorMessages.unpackSubnetToL1ConversionMessage(_getPChainWarpMessage(messageIndex).payload);
   ...
   */
  function _initializeValidatorRegistration(ValidatorRegistrationInput calldata input, uint64 weight)
    internal
    virtual
    ...
    returns (bytes32)
  {
    ...
    emit ValidationPeriodCreated(validationID, input.nodeID, messageID, weight, input.registrationExpiry);
  ...
  function completeValidatorRegistration(uint32 messageIndex) external {
    ...
    (bytes32 validationID, bool validRegistration) =
      ValidatorMessages.unpackL1ValidatorRegistrationMessage(_getPChainWarpMessage(messageIndex).payload);
    ...
    $._validationPeriods[validationID].status = ValidatorStatus.Active;
    $._validationPeriods[validationID].startedAt = uint64(block.timestamp);
    emit ValidationPeriodRegistered(validationID, $._validationPeriods[validationID].weight, block.timestamp);
  }
   ...
   */
  function _initializeEndValidation(bytes32 validationID) internal virtual returns (Validator memory) {
    ValidatorManagerStorage storage $ = _getValidatorManagerStorage();
  ...
  function resendEndValidatorMessage(bytes32 validationID) external {
    ...
    WARP_MESSENGER.sendWarpMessage(ValidatorMessages.packL1ValidatorWeightMessage(validationID, validator.messageNonce, 0));
  }
  ...
  function _completeEndValidation(uint32 messageIndex) internal returns (bytes32, Validator memory) {
    ...
    if (validator.status != ValidatorStatus.PendingRemoved && validator.status != ValidatorStatus.PendingAdded) {
  ...
  function _setValidatorWeight(bytes32 validationID, uint64 newWeight) internal returns (uint64, bytes32) {
    ValidatorManagerStorage storage $ = _getValidatorManagerStorage();
    ...
    // Submit the message to the Warp precompile.
    bytes32 messageID = WARP_MESSENGER.sendWarpMessage(ValidatorMessages.packL1ValidatorWeightMessage(validationID, nonce, newWeight));

    emit ValidatorWeightUpdate({validationID: validationID, nonce: nonce, weight: newWeight, setWeightMessageID: messageID});
   ...
   */
  function _checkAndUpdateChurnTracker(uint64 newValidatorWeight, uint64 oldValidatorWeight) private {
    ValidatorManagerStorage storage $ = _getValidatorManagerStorage();
    ...
    if (churnTracker.startedAt == 0 || currentTime >= churnTracker.startedAt + $._churnPeriodSeconds) {
