...
import { MockChainlinkPriceFeed } from "../tests/mocks/MockChainlinkPriceFeed.sol";
...
contract UpgradeNodeAsAService is Script {
  ...
  function run() external {
    ...
    if (!hasAdminRole) {
      ...
      console.log(
        "SOLUTION: Use an account with admin privileges or grant admin role to current account."
      );
      console.log(
        "To find admin accounts, run: forge script script/upgradeNodeAsAService.s.sol:UpgradeNodeAsAService --sig 'checkAdminRole()' --fork-url $ETH_RPC_URL"
      );
    ...
    MockChainlinkPriceFeed mockPriceFeed = new MockChainlinkPriceFeed(2000000000);
    console.log("Mock price feed deployed at:", address(mockPriceFeed));

    // Prepare initialization data for V2
    address avaxPriceFeed;
    if (block.chainid == 43113) {
      avaxPriceFeed = address(mockPriceFeed);
    } else {
      avaxPriceFeed = 0x0A77230d17318075983913bC2145DB16C7366156;
    }
    ...
    bytes memory initData =
      abi.encodeWithSelector(NodeAsAService.initializeV2.selector, avaxPriceFeed, payAsYouGoFee);
